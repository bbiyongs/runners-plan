...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if(StringUtils.hasText(token) && jwtTokenProvider.validateToken(token)) {
            Long runnerId = jwtTokenProvider.getRunnerIdFromToken(token);

            // securitycontext 에 저장할 authentication 객체 생성
            UsernamePasswordAuthenticationToken authentication =
                    new UsernamePasswordAuthenticationToken(runnerId, null, Collections.singleton(new SimpleGrantedAuthority("ROLE_USER")));
    ...
    // header 에서 authorization : bearer <token> 파싱
