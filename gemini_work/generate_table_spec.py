...
def create_db_table_grid(doc, headers, data, col_widths):
    ...
    for r_idx, row_data in enumerate(data):
        ...
        for c_idx, text in enumerate(row_data):
            ...
            # Alignments: Field, Data Type, Key are centered; Field2, Domain are left/center
            if c_idx in (0, 2, 3, 4):
                p.alignment = WD_ALIGN_PARAGRAPH.CENTER
            else:
...
def main():
    ...
    for section in doc.sections:
        ...
        hrun = hp.add_run("Running Coach — 데이터베이스 테이블 명세서 v1.0")
    ...
    sub_run = subtitle_p.add_run("Running Coach 데이터 모델 설계 산출물")
    ...
    # RUN_RECORD (Added logically based on relationships)
    add_heading_2(doc, "RUN_RECORD (러닝 기록)")
    run_record_data = [
        ("run_record_id", "러닝기록ID", "PK", "BIGINT", "PK"),
        ("runner_id", "러너ID", "FK", "BIGINT", "FK"),
        ("run_date", "운동일자", "DATE", "DATE", ""),
        ("distance", "거리(km)", "DISTANCE", "NUMERIC(10,2)", ""),
        ("run_time_sec", "운동시간(초)", "RECORD", "INTEGER", ""),
        ("average_pace", "평균페이스", "PACE", "VARCHAR(20)", ""),
        ("average_heart_rate", "평균심박수", "HEART_RATE", "INTEGER", ""),
        ("training_type_code", "훈련종류코드", "CODE", "VARCHAR(30)", ""),
        ("rpe", "훈련강도(RPE)", "RPE", "INTEGER", ""),
        ("temperature", "온도", "TEMP", "NUMERIC(5,1)", ""),
        ("humidity", "습도", "HUMID", "INTEGER", ""),
    ...
    r_erd = p_erd.add_run(
        "■ 테이블 간 계층 관계\n"
        "RUNNER (러너 정보)\n"
        " ├── RUNNER_SOCIAL_ACCOUNT (소셜 계정)\n"
        " ├── RUN_RECORD (러닝 기록)\n"
        " ├── GOAL (목표 관리)\n"
        " └── RACE_RECORD (대회 기록)\n\n"
        "CODE_GROUP (공통 코드 그룹)\n"
        " └── CODE_DETAIL (공통 코드 상세)\n\n"
        "■ 공통 코드 매핑 관계\n"
        "RUN_RECORD (러닝 기록)\n"
    ...
    for group in ["TRAINING_TYPE (훈련 유형)", "WEATHER_TYPE (날씨 유형)", "GOAL_TYPE (목표 유형)", "RACE_TYPE (대회 유형)", "RUNNING_LEVEL (러닝 레벨)"]:
        add_custom_bullet(doc, group)
        
    doc.add_paragraph().paragraph_format.space_after = Pt(6)
        
    # Code Details: GOAL_TYPE
    add_heading_2(doc, "GOAL_TYPE (목표 유형 상세)")
    goals = [
    ...
    add_heading_2(doc, "TRAINING_TYPE (훈련 유형 상세)")
    trainings = [
    ...
    add_heading_2(doc, "WEATHER_TYPE (날씨 유형 상세)")
    weathers = [
        "SUNNY: 맑음",
        "CLOUDY: 흐림",
        "RAIN: 비",
    ...
    filename = "c:/Runners_plan/산출물/테이블_명세서_Running_Coach_v1.0.docx"
    doc.save(filename)
    print(f"Successfully created Table Specification document '{filename}'")
