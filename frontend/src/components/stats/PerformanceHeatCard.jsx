        ...
        <div className="dashboard-widget-card">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(145px, 1fr))', gap: '8px', marginTop: '12px' }}>
                {points && points.slice(0, 6).map((pt, idx) => (
                    <div key={idx} style={{ background: '#f8faf8', border: '1px solid #dbe7df', padding: '10px 12px', borderRadius: '8px' }}>
                        <div style={{ fontSize: '0.78rem', fontWeight: 'bold', color: '#4a6356', display: 'flex', alignItems: 'center', gap: '4px' }}>
                            <Clock size={13} color="#2e7d32" /> {pt.weekday}요일 {pt.time_slot}
                        </div>
                        <div style={{ fontSize: '1rem', fontWeight: '800', color: '#192e24', marginTop: '4px' }}>
                            {formatPace(pt.avg_pace_sec)}
                        </div>
                        {/* 💡 요일 전체 횟수 및 비중 (%) 표출 */}
                        <div style={{ fontSize: '0.72rem', color: '#8aa395', marginTop: '4px', borderTop: '1px solid #dbe7df', paddingTop: '4px' }}>
