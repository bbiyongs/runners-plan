...
def connect_garmin_account(req: GarminConnectRequest, db: Session = Depends(get_db)):
    """
...
def sync_initial_history(runner_id: int, db: Session = Depends(get_db)):
    """
    ...
    try:
        service = GarminSyncService(db)
        saved_count = service.sync_initial_history(runner_id)
        return GarminSyncResponse(
            success=True,
            message=f"초기 동기화 완료 (총 {saved_count}개 저장)",
            saved_count=saved_count
        )
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"초기 동기화 중 오류 발생: {str(e)}")


@router.post("/sync-recent/{runner_id}", summary="최근 데이터 수동 동기화 ")
def sync_recent_activities(runner_id:int, limit: int=10, db: Session=Depends(get_db)):
    """
